...
class ContentService {
  ...
  private fetchEntriesForContentType(contentTypeSysId: string) {
  ...
  public getSinglePortfolioItem(slug: string) {
    return new Promise((resolve, reject) => {
      this.fetchEntriesForContentType('portfolioItem').then((items: any[]) => {
        const matchedItem = items.find((item: any) => item.fields.slug === slug);
