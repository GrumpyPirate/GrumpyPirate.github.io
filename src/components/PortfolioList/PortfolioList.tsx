...
const PortfolioList: FunctionComponent = function () {
  const { data: { portfolioItemCollection: { items: portfolioItems = [] } = {} } = {} } =
    useQuery<GetPortfolioItemsResponse>(GET_PORTFOLIO_ITEM_LIST);
  const [currentSortingStrategy, setCurrentSortingStrategy] =
  ...
  const onSelectChange = useCallback((option: SingleValue<{ value: string; label: string }>) => {
